...
import os
import numpy as np
import utils_maps_2024 as um
import Old.utils_contours as uc
...
class gcode_manager():
    ...
    def generate_laser_focusing_test_gcode(self, file, feed_rate = 1000):
        Lx = 10
        dy = 2
        nlines = 11 #make odd so nominal height is middle. 
        dz = 2
        ...
        for j in range(nlines):
            self.laseron()
            self.add_path_to_gcode([0, Lx], [cury, cury], [curz, curz])
            self.laseroff()
            curz += dz
            cury += dy
    ...
    def laseron(self, intensity=1):
        if intensity>1 or intensity<0:
            raise ValueError('Laser intensity must be between 0 and 1 inclusive')
        
        self.gcode += f'S{intensity*250}\n'
        return
    ...
    def add_path_to_gcode(self, xpoints, ypoints, zpoints, start_safe=False):
        # Start_safe is specifically for rastering passes (roughing) so tool does not
